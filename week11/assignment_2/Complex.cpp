...
Complex::Complex() {
     ...
     */
	this->value = std::pair<double, double> { 0, 0 };
}
...
Complex::Complex(float _val1, float _val2) {
     ...
     */
	this->value = std::pair<double, double>{ _val1, _val2 };
...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* test = dynamic_cast<Integer*>(num);
		Number* a= new Complex( this->value.first + test->val(), this->value.second );
		delete(num);
		return a;

	}
	else if (num->types() == FLOAT) {
		Float* test = dynamic_cast<Float*>(num);
		Number* a = new Complex(this->value.first + test->val(), this->value.second);
		delete(this);
		return a;
		...
		Complex* test = dynamic_cast<Complex*>(num);
		Number* a = new Complex(this->value.first + test->val().first, this->value.second + test->val().second);
		delete(this);
		return a;
	}
    return nullptr;
...
Number *Complex::sub(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* test = dynamic_cast<Integer*>(num);
		Number* a = new Complex(this->real() - test->val(), this->imag());
		delete(num);
		return a;
		...
		Complex* test = dynamic_cast<Complex*>(num);
		Number* a = new Complex(this->real() - test->real(), this->imag() - test->imag());
		delete(this);
		return a;
	}
    return nullptr;
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* test = dynamic_cast<Integer*>(num);
		Number* a = new Complex(this->real()* test->val(), this->imag() * test->val());
		delete(num);
		return a;
		...
		Complex* test = dynamic_cast<Complex*>(num);
		Number* a = new Complex((this->real()*test->real()) - (test->imag() * this->imag()), (this->real()*test->imag()) + (this->imag()*test->real()));
		delete(this);
		return a;
	}
    return nullptr;
...
Number *Complex::div(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* test = dynamic_cast<Integer*>(num);
		Number* a = new Complex(this->value.first / test->val(), this->value.second / test->val());
		delete(num);
		return a;
...
void Complex::set_val(double real, double imag) {
     ...
     * 이 때 make_pair함수를 이용
     */
	this->value = std::pair<double, double>{ real, imag };
}
